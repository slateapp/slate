...
class RequestsController < ApplicationController
  before_action :authenticate!, only: %i[new create destroy edit]
  before_action :deny_to_unapproved, only: %i[new create edit update destroy]

  def index
    @requests = Request.for_cohort(
      (params[:cohort] || current_user.try(:cohort)) || Cohort.all
    ).where(solved: false).sort do |a, b|
a.created_at <=> b.created_at end
  end
  ...
  def new
    if current_student
      ...
      (redirect_to root_path, notice: 'Sorry, you must be a student to make a request!')
    end
  end
  ...
  def update
    ...
    update_and_trigger_websocket(
      params[:request].permit(:description, :category, :solved), 'edit', @request,
      :request_edited, students_dashboard_path
    )
  ...
  def display
    if Cohort.where(selected: true).count != 2
      redirect_to root_path,
                  notice: 'Please ask a teacher to select current cohorts.'
    end
    @cohort_one = Cohort.current_cohorts.first
    @cohort_two = Cohort.current_cohorts.second
    @requests_for_cohort_one = get_requests(@cohort_one)
    @requests_for_cohort_two = get_requests(@cohort_two)
  end
end
